...
public class LightControllerStateMachine implements
	...
	private void updateState(int state) {
		for (LightControllerStateMachineObserverInterface obs : this.observers) {
			 ...
			 * BUG
			 * AK
			 * updateLightState(state) was being called twice, only supposed to be called once.
			 */
			//obs.updateLightState(state);
		}
	}
	...
	protected void handleEntryConditions(int presentState) {
		...
		case LightControllerStateMachineObserverInterface.MOTION_DETECTED:
			 ...
			 * BUG
			 * AK
			 * break; was commented out.
			 * Uncommented break statement.
			 */
			break;
	...
	public void signalAction(int request) {
		 ...
		 * BUG
		 * AK
		 * stateChange was initialized to true, could never be false.
		 * Set to true so that it is actually used correctly.
		 */
		 ...
		 * BUG
		 * AK
		 * Was originally setting stateChange to true,
		 * Changed to == from =
		 */
		if (stateChange == true) {
			// Invoke exit actions.
